...
export async function POST(req: Request) {
  try {
    ...
    // ── 1. MongoDB first (seeded problems) ────────────────────────────────────
    try {
    ...
    // ── 2. Build company profile + RAG examples ───────────────────────────────
    ...
    // ── 3. No AI — return static examples directly ────────────────────────────
    if (!process.env.GROQ_API_KEY && !process.env.OPENAI_API_KEY) {
      return NextResponse.json({
        questions: staticExamples.slice(0, count).map((p, i) => ({
          id: i + 1, title: p.title, difficulty: p.difficulty, topic: p.pattern,
          statement: p.desc ?? p.statement ?? "",
          constraints: Array.isArray(p.constraints) ? p.constraints.join("\n") : (p.constraints ?? ""),
          example: p.examples?.[0] ?? { input: "", output: "", explanation: "" },
          hints: [`Pattern: ${p.pattern}`],
        })),
        company: companyName, section: sectionId, source: "static_bank",
      })
    }
